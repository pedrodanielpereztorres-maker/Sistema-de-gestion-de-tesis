...
from ..estado.estado_mantenimiento import EstadoMantenimiento, TutorAcademico, Rol, UsuarioSistema, Carrera
...
def barra_herramientas(titulo: str, placeholder_busqueda: str, valor_busqueda: rx.Var, setter_busqueda, texto_boton: str, icono_boton: str, accion_boton) -> rx.Component:
    return rx.hstack(
        rx.vstack(
            rx.text(titulo, font_weight="800", color="#1E293B",
                    font_size="18px", letter_spacing="-0.01em"),
            rx.text("Administración y gestión de registros",
                    color="#334155", font_size="12px", font_weight="500"),
            spacing="0", align="start"
        ),
        rx.spacer(),
        rx.hstack(
            rx.box(
                rx.icon("search", size=15, color="#000000", position="absolute",
                        left="12px", top="50%", transform="translateY(-50%)", z_index="1"),
                rx.input(
            ...
            rx.button(
                rx.hstack(rx.icon(icono_boton, size=18),
        ...
        padding_bottom="20px", border_bottom=f"1px solid {COLOR_BORDE}"
    )


def celda_cabecera(texto: str) -> rx.Component:
    return rx.table.column_header_cell(
        rx.text(texto, font_weight="700", font_size="11px",
                color="#475569", letter_spacing="0.05em"),
        padding_y="16px",
        padding_x="20px",
        background="#F1F5F9",
...
def boton_accion(icono: str, color: str, accion, tooltip: str) -> rx.Component:
    return rx.icon_button(
        ...
        on_click=accion,
        variant="solid", color_scheme=color,
        size="2", radius="full", cursor="pointer",
        title=tooltip
    )
        ...
        rx.table.cell(
            rx.hstack(
                ...
                rx.center(
                    ...
                    flex_shrink="0",
                    background=rx.cond(
                        user.rol == "Administrador", "#4F46E5", "#059669"  # Colores un poco más intensos
                    ),
                ),
                ...
                rx.vstack(
                    rx.text(
                        ...
                        font_weight="600", font_size="13px", color="#0F172A"
                    ),
        ...
        rx.table.cell(
            rx.badge(
                ...
                color_scheme=rx.cond(
                    user.rol == "Administrador", "indigo", "green"),
                variant="soft",
                radius="full",
                size="2",
                style={
                    ...
                    "background_color": rx.cond(user.rol == "Administrador", "#E0E7FF", "#D1FAE5"),
                    "font_weight": "bold",
                    "padding_x": "10px",
                }
            ),
        ...
        rx.table.cell(
            rx.hstack(
                rx.badge(
                    ...
                    variant="soft", radius="full", font_weight="bold"
                ),
                rx.icon_button(
                    rx.icon(rx.cond(user.esta_activo,
                            "lock", "unlock"), size=14),
...
def contenido_usuarios() -> rx.Component:
    return rx.vstack(
        barra_herramientas(
            "Cuentas de Acceso", "Buscar usuario...",
            EstadoMantenimiento.busqueda_usuarios, EstadoMantenimiento.fijar_busqueda_usuarios,
            "Nuevo Usuario", "user-plus", EstadoMantenimiento.abrir_modal_usuario
        ),
        contenedor_tabla(
            rx.table.root(
                rx.table.header(
            ...
            rx.hstack(
                rx.button(
                    ...
                    on_click=lambda: EstadoMantenimiento.abrir_modal_carrera(
                        True, carrera),
                    size="2", variant="solid", color_scheme="blue"
                ),
                rx.cond(
                    carrera.tiene_movimientos,
                    rx.button(
                        rx.icon(rx.cond(carrera.esta_activa,
                                "power-off", "power"), size=16),
                        ...
                        color_scheme=rx.cond(
                            carrera.esta_activa, "orange", "green"),
                        style={
...
def contenido_carreras() -> rx.Component:
    return rx.vstack(
        barra_herramientas(
            "Carreras Universitarias", "Buscar carrera...",
            EstadoMantenimiento.busqueda_carreras, EstadoMantenimiento.fijar_busqueda_carreras,
            "Nueva Carrera", "plus", lambda: EstadoMantenimiento.abrir_modal_carrera()
        ),
        contenedor_tabla(
            rx.table.root(
                rx.table.header(
...
def fila_tutor(tutor: TutorAcademico) -> rx.Component:
    return rx.table.row(
        ...
        rx.table.cell(
            rx.hstack(
                # Botón Editar
                boton_accion("pencil", "amber",
                             lambda: EstadoMantenimiento.abrir_modal_tutor(True, tutor), "Editar"),
                ...
                rx.cond(
                    ...
                    rx.icon_button(
                        rx.icon(
                            rx.cond(tutor.activo, "power-off", "power"), size=16),
                    ...
                    boton_accion("trash-2", "red",
                                 lambda: EstadoMantenimiento.eliminar_tutor(tutor.id), "Eliminar Definitivamente")
                ),
                spacing="2"
            )
        ),
...
def contenido_tutores() -> rx.Component:
    return rx.vstack(
        barra_herramientas(
            "Tutores Académicos", "Buscar tutor...",
            EstadoMantenimiento.busqueda_tutores, EstadoMantenimiento.fijar_busqueda_tutores,
            "Nuevo Tutor", "user-plus", lambda: EstadoMantenimiento.abrir_modal_tutor(
                False, None)
        ),
        rx.box(
            rx.hstack(
...
def fila_rol(rol: Rol) -> rx.Component:
    return rx.table.row(
        ...
        rx.table.cell(
            rx.hstack(
                boton_accion("trash-2", "red",
                             lambda: EstadoMantenimiento.abrir_confirmacion_rol(rol.id), "Eliminar Rol")
            ),
            padding_y="16px", padding_x="20px"
        ),
        _hover={"background": "rgba(0,0,0,0.02)"},
...
def contenido_roles() -> rx.Component:
    return rx.vstack(
        barra_herramientas(
            "Roles y Permisos", "Buscar rol...",
            EstadoMantenimiento.busqueda_roles, EstadoMantenimiento.fijar_busqueda_roles,
            "Crear Rol", "shield-plus", EstadoMantenimiento.abrir_modal_rol
        ),
        contenedor_tabla(
            rx.table.root(
                rx.table.header(
...
def cabecera_modal(titulo: str, subtitulo: str, icono: str, on_close: rx.EventHandler) -> rx.Component:
    return rx.hstack(
        rx.center(
        ...
        rx.vstack(
            rx.text(titulo, font_size="18px",
                    font_weight="800", color="#1E293B"),
        ...
        rx.icon_button(
            ...
            on_click=on_close, cursor="pointer"
        ),
...
def pie_modal(texto_guardar: str, accion_guardar: rx.EventHandler, accion_cancelar: rx.EventHandler) -> rx.Component:
    return rx.hstack(
        rx.button("Cancelar", variant="soft", color_scheme="gray",
                  on_click=accion_cancelar, radius="large"),
        rx.button(
            texto_guardar, on_click=accion_guardar,
            radius="large", variant="solid", color_scheme="indigo",
            background=DEGRADADO_ICONO,
            box_shadow="0 4px 12px rgba(99,102,241,0.3)"
        ),
        justify="end", width="100%", margin_top="24px",
        padding_top="20px", border_top=f"1px solid {COLOR_BORDE}"
    )
...
def modal_usuario() -> rx.Component:
    return rx.cond(
        ...
        rx.box(
            rx.box(position="fixed", inset="0", bg="rgba(10,10,30,0.5)", backdrop_filter="blur(4px)", z_index="200",
                   on_click=EstadoMantenimiento.cerrar_modal_usuario),
            rx.box(
                rx.vstack(
                    cabecera_modal(
                        "Nuevo Usuario", "Registra un nuevo usuario en el sistema", "user-plus",
                        EstadoMantenimiento.cerrar_modal_usuario
                    ),
                    rx.grid(
                        campo_texto("Cédula de Identidad", EstadoMantenimiento.u_cedula,
                                    EstadoMantenimiento.fijar_u_cedula),
                        columns="1", width="100%"
                    ),
                    rx.grid(
                        campo_texto("Nombre", EstadoMantenimiento.u_nombre,
                                    EstadoMantenimiento.fijar_u_nombre),
                        campo_texto("Apellido", EstadoMantenimiento.u_apellido,
                                    EstadoMantenimiento.fijar_u_apellido),
                        columns="2", spacing="4", width="100%"
                    ),
                    campo_texto("Correo", EstadoMantenimiento.u_correo,
                                EstadoMantenimiento.fijar_u_correo, tipo="email"),

                    rx.grid(
                        campo_texto("Contraseña", EstadoMantenimiento.u_clave,
                                    EstadoMantenimiento.fijar_u_clave, tipo="password"),
                        rx.vstack(
                            rx.text("Rol Asignado", font_size="13px",
                                    font_weight="600", color="#0F172A"),
                            rx.select(EstadoMantenimiento.nombres_roles,
                                      on_change=EstadoMantenimiento.fijar_u_rol, width="100%",
                                      radius="large", size="3",
                                      variant="classic",
                                      color="black",
                                      style={
                    ...
                    pie_modal("Crear Cuenta", EstadoMantenimiento.guardar_usuario,
                              EstadoMantenimiento.cerrar_modal_usuario),

                    width="100%", spacing="5"
                ),
                padding="24px", background="white", border_radius="16px",
                box_shadow=SOMBRA_MODAL, width="550px", z_index="210",
    ...
    return rx.cond(
        ...
        rx.box(
            ...
            rx.box(
                rx.vstack(
                    cabecera_modal(
                        rx.cond(EstadoMantenimiento.c_en_edicion,
                                "Editar Carrera", "Nueva Carrera"),
                        "Gestiona las carreras universitarias", "graduation-cap",
                        EstadoMantenimiento.cerrar_modal_carrera
                    ),
                    campo_texto("Nombre de la Carrera", EstadoMantenimiento.c_nombre,
                                EstadoMantenimiento.fijar_c_nombre),

                    pie_modal(rx.cond(EstadoMantenimiento.c_en_edicion, "Guardar Cambios", "Crear Carrera"),
                ...
                padding="24px", background="white", border_radius="16px",
                box_shadow=SOMBRA_MODAL, width="450px", z_index="210",
                position="fixed", top="50%", left="50%", transform="translate(-50%, -50%)",
                on_click=rx.stop_propagation
            )
        )
    ...
    return rx.cond(
        ...
        rx.box(
            ...
            rx.box(
                rx.vstack(
                    cabecera_modal(
                        rx.cond(EstadoMantenimiento.t_en_edicion,
                                "Editar Tutor", "Nuevo Tutor"),
                        "Gestiona la información del personal académico", "graduation-cap",
                        EstadoMantenimiento.cerrar_modal_tutor
                    ),

                    rx.grid(
                        campo_texto("Cédula", EstadoMantenimiento.t_cedula,
                                    EstadoMantenimiento.fijar_t_cedula,
                                    on_blur=EstadoMantenimiento.cargar_datos_usuario_tutor),
                        campo_texto("Nombre Completo", EstadoMantenimiento.t_nombre,
                                    EstadoMantenimiento.fijar_t_nombre),
                        columns="2", spacing="4", width="100%"
                    ),

                    rx.grid(
                        campo_texto("Correo Electrónico", EstadoMantenimiento.t_correo,
                                    EstadoMantenimiento.fijar_t_correo, tipo="email"),
                        campo_texto("Teléfono", EstadoMantenimiento.t_telefono,
                                    EstadoMantenimiento.fijar_t_telefono, tipo="tel"),
                        columns="2", spacing="4", width="100%"
                    ),

                    rx.grid(
                        rx.vstack(
                            rx.text("Carrera", font_size="13px",
                                    font_weight="600", color="#0F172A"),
                            rx.select(
                        ...
                        campo_texto("Especialidad", EstadoMantenimiento.t_especialidad,
                                    EstadoMantenimiento.fijar_t_especialidad),
                        columns="2", spacing="4", width="100%"
                    ),

                    pie_modal("Guardar Datos", EstadoMantenimiento.guardar_tutor,
                              EstadoMantenimiento.cerrar_modal_tutor),
                    width="100%", spacing="5"
                ),
                padding="24px", background="white", border_radius="16px",
                box_shadow=SOMBRA_MODAL, width="500px", z_index="210",
                position="fixed", top="50%", left="50%", transform="translate(-50%, -50%)",
                on_click=rx.stop_propagation
            )
        )
    ...
    return rx.cond(
        ...
        rx.box(
            ...
            rx.box(
                rx.vstack(
                    cabecera_modal(
                        "Nuevo Rol", "Define nuevos niveles de acceso", "shield-check",
                        EstadoMantenimiento.cerrar_modal_rol
                    ),
                    campo_texto("Nombre del Rol", EstadoMantenimiento.r_nombre,
                                EstadoMantenimiento.fijar_r_nombre),
                    campo_texto("Descripción", EstadoMantenimiento.r_descripcion,
                                EstadoMantenimiento.fijar_r_descripcion),

                    pie_modal("Guardar Rol", EstadoMantenimiento.guardar_rol,
                              EstadoMantenimiento.cerrar_modal_rol),
    ...
    return rx.cond(
        ...
        rx.box(
            ...
            rx.box(
                rx.vstack(
                    cabecera_modal(
                        "Confirmar Eliminación", "Esta acción requiere autorización", "shield-alert",
                        EstadoMantenimiento.cerrar_confirmacion_rol
                    ),
                    rx.text("Para eliminar este rol, confirme su contraseña de administrador:",
                            color="#334155", font_size="13px", font_weight="500"),

                    campo_texto("Contraseña del Administrador", EstadoMantenimiento.password_confirmacion,
                                EstadoMantenimiento.fijar_password_confirmacion, tipo="password"),

                    rx.hstack(
                        rx.button("Cancelar", variant="soft", color_scheme="gray",
                                  on_click=EstadoMantenimiento.cerrar_confirmacion_rol, radius="large"),
                        rx.button(
                            "Eliminar Definitivamente", on_click=EstadoMantenimiento.confirmar_eliminar_rol,
                            radius="large", variant="solid", color_scheme="red",
...
def contenido_mantenimiento() -> rx.Component:
    ...
    return rx.vstack(
        encabezado_pagina(
            "Mantenimiento del Sistema", "", None),

        rx.tabs.root(
            rx.tabs.list(
                rx.tabs.trigger(
                    "Usuarios", value="usuarios",
                    padding_x="20px", padding_y="10px", color="#334155",
                    _active={
                        "color": COLOR_PRIMARIO, "background": "white",
                        "border_bottom": f"2px solid {COLOR_PRIMARIO}",
                        "font_weight": "700"
                    }
                ),
                rx.tabs.trigger(
                    "Tutores Académicos", value="tutores",
                    padding_x="20px", padding_y="10px", color="#334155",
                    _active={
                        "color": COLOR_PRIMARIO, "background": "white",
                        "border_bottom": f"2px solid {COLOR_PRIMARIO}",
        ...
        toast_viewer(),
        modal_carrera(),
        width="100%", padding=["20px", "28px", "28px", "36px"],
        max_width="1200px",
        ...
        rx.cond(
            ...
            rx.center(
                rx.vstack(
                    ...
                    rx.heading("Acceso Restringido",
                               size="6", color="#1E293B"),
                    rx.text(
                        "Solo los administradores pueden ver esta página.", color="#334155"),
